...
<style type="text/css">
/* 对全部标签对象进行初始化操作 */
*::selection { /*即点击全部的任何标签插件时都不会有背景颜色，因为默认的标签对象当中当进行多次点击操作时会出现背景色*/
	background: none;
}
...
#box {
	...
	margin: 0 auto; /*设置box标签对象的上边距为100像素，左右为自动居中操作*/
	margin-left: 20px;
	margin-right: 20px;
	background: #663366;
...
.img-li { /*对无序列表标签对象当中样式进行设置操作*/
	/*对无序列表标签当中的数据的宽高都设置为100%即和box当中的长宽是相互吻合的*/
	width: 590px;
	height: 385px;
...
.img-li:FIRST-CHILD { /*将无序列表项当中的第一个子节点对象当中的图片的内容设置为不透明的*/
	opacity: 1;
}
/*对img-li标签对象的第一个子节点表标签对象结束样式设置*/

/*对图片当中的两个切换图片的按钮开始样式设置*/
.img-button { /*对button类当中的样式格式内容进行设置操作*/
	width: 80px;
	height: 30px;
...
.button-ul { /*设置ul无序列表标签对象的高度和宽度,以及ul标签对象当中的背景颜色*/
	height: 22px;
	width: 80px;
	/*ul标签对象当中进行存放内容的区域依然为一个长方形采用border属性值可以将之修改为一个圆角矩形*/
	/*background: #000;*/
	background-color: rgba(0, 0, 0, 0.8);
	/*对ul标签对象的背景颜色进行设置操作，前三个参数为rgb所对应的颜色参数最后一个0.5则是用于设置当前颜色的透明度*/
	padding: 5px 25px; /*对ul标签对象进行内边距的设置操作，将上下设置为5px，左右为25px*/
	border-radius: 15px; /*设置圆角的半径*/
	position: absolute;
	/*将ul标签对象设置为绝对定位的方式，为了让ul标签可以移动到盒子当中所显示的图片的指定的位置当中去*/
...
.button-ul li {
	...
	border-radius: 5px;
	/*进行边框半径的设置操作，将当前li标签对象当中进行内容输入的正方形区域设置为一个半径为5的圆形区域,即将li标签对象的内容输入区域的边框设置为圆脚区域*/
	cursor: pointer; /*cursor(光标)*,即当光标移动到了设置了button样式的标签对象当中之后将会变为手形的光标*/
}
/*---------------------当前样式设置完毕-------------------------------*/
...
<body>
	...
	<div id="box">
		<ul>
			...
			<li class="img-li"><img src="photo/wenju3.jpg"></li>
			<li class="img-li"><img src="photo/wenju1.jpg"></li>
		</ul>
